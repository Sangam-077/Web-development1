...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'submit_review') {
        ...
        try {
            ...
            $stmt = $conn->prepare("INSERT INTO review (review_id, customer_id, product_id, rating, comment, approved, review_time) VALUES (?, ?, ?, ?, ?, ?, NOW())");
            if ($stmt === false) {
                throw new Exception("Prepare failed: " . $conn->error);
            }
...
try {
    $stmt = $conn->prepare("SELECT * FROM review ORDER BY review_time DESC LIMIT 3");
    if ($stmt === false) {
        throw new Exception("Prepare failed: " . $conn->error);
    }
    $stmt->execute();
