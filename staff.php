...
$recentOrders = [];
try {
    $stmt = $conn->query("SELECT o.order_id, u.name as customer, o.total_price, o.order_type as type, o.order_time, s.name as assigned_staff, o.staff_id FROM orders o JOIN users u ON o.customer_id = u.user_id LEFT JOIN users s ON o.staff_id = s.user_id ORDER BY o.order_time DESC LIMIT 5");
    if ($stmt === false) {
...
<body>
    <div class="sidebar">
        ...
        <nav>
            ...
            <a href="#internal-messages" class="nav-item"><i class="fas fa-comments"></i> Internal Messages</a>
    ...
    <div class="main-content">
        ...
        <div class="dashboard-bottom">
            ...
            <div class="alerts">
                ...
                <div class="system-status">
                    ...
                    <p>All systems operational. Last backup: Sep 23, 2025 2:13 AM</p>
                </div>
            </div>
        </div>
        ...
        <section id="internal-messages" style="padding: 80px 0; background: var(--background-cream);">
            <div class="container">
                <h2 style="font-family: 'Playfair Display', serif; font-size: 32px; color: var(--text-dark);">Internal Messages</h2>
                <form method="post" class="staff-form">
                    <input type="hidden" name="action" value="send_message">
                    <label>Recipient:</label>
                    <select name="recipient_id" required>
                        <?php
                        $stmt = $conn->query("SELECT user_id, name FROM users WHERE role IN ('staff', 'admin', 'cashier') AND user_id != '" . $_SESSION['user_id'] . "'");
                ...
                <table class="staff-table">
                    ...
                    <tbody>
                        <?php
                        $stmt = $conn->prepare("SELECT notification_id, content, sent_time, is_read FROM notification WHERE user_id = ? AND notif_type = 'message' AND content NOT LIKE 'Contact Form Submission:%' ORDER BY sent_time DESC");
                        $stmt->bind_param("s", $_SESSION['user_id']);
                        $stmt->execute();
                        $result = $stmt->get_result();
        ...
        <!-- Reviews Section -->
        <section id="reviews" style="padding: 80px 0; background: var(--background-cream);">
            <div class="container">
                ...
                <table class="staff-table">
                    ...
                    <tbody>
                        <?php
                        try {
                            $stmt = $conn->query("SELECT r.review_id, u.name as customer, p.name as product, r.rating, r.comment, r.review_time, r.approved FROM review r LEFT JOIN users u ON r.customer_id = u.user_id LEFT JOIN product p ON r.product_id = p.product_id ORDER BY r.review_time DESC");
                            if ($stmt === false) {
                                error_log("Query failed: " . $conn->error);
                            } else {
                                if ($stmt->num_rows === 0) {
                                    ...
                                    while ($review = $stmt->fetch_assoc()) {
                                        ...
                                        echo "<td>" . htmlspecialchars($review['customer'] ?? 'Unknown') . "</td>";
                                        echo "<td>" . htmlspecialchars($review['product'] ?? 'General') . "</td>";
                                        echo "<td>" . htmlspecialchars($review['rating']) . "</td>";
                                        echo "<td>" . htmlspecialchars($review['comment'] ?? 'No comment') . "</td>";
                                        echo "<td>" . htmlspecialchars($review['review_time']) . "</td>";
                            ...
                            echo "<tr><td colspan='8'>Error fetching reviews</td></tr>";
                        }
                        ?>
                    </tbody>
                </table>
